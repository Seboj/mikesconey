...
function cmdStateLoad(cwd, raw) {
  ...
  let stateRaw = '';
  try {
    stateRaw = fs.readFileSync(path.join(canopyDir, 'STATE.md'), 'utf-8');
  } catch {}

  const configExists = fs.existsSync(path.join(canopyDir, 'config.json'));
  const roadmapExists = fs.existsSync(path.join(canopyDir, 'ROADMAP.md'));
