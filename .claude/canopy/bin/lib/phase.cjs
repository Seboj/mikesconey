...
function cmdFindPhase(cwd, phase, raw) {
  ...
  try {
    ...
    const result = {
      found: true,
      directory: path.join('.canopy', 'phases', match),
...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  const dirPath = path.join(cwd, '.canopy', 'phases', dirName);
  ...
  // Build phase entry
  const phaseEntry = `\n### Phase ${newPhaseNum}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${maxPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /canopy:plan-phase ${newPhaseNum} to break down)\n`;
  ...
  const result = {
    ...
    name: description,
    slug,
    directory: `.canopy/phases/${dirName}`,
  };
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  // Build phase entry
  const phaseEntry = `\n### Phase ${decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /canopy:plan-phase ${decimalPhase} to break down)\n`;
...
function cmdPhaseRemove(cwd, targetPhase, options, raw) {
  ...
  const statePath = path.join(cwd, '.canopy', 'STATE.md');
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  if (fs.existsSync(roadmapPath)) {
    ...
    const reqPath = path.join(cwd, '.canopy', 'REQUIREMENTS.md');
