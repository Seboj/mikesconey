...
/**
 * CANOPY Tools — CLI utility for Canopy workflow operations
 *
 * Replaces repetitive inline bash patterns across ~50 Canopy command/workflow/agent files.
 * Centralizes: config parsing, model resolution, phase lookup, git commits, summary verification.
 *
 * Usage: node canopy-tools.cjs <command> [args] [--raw]
 *
 * Atomic Commands:
 ...
 *   config-ensure-section              Initialize .canopy/config.json
 ...
 *   validate health [--repair]         Check .canopy/ integrity, optionally repair
 *
 * Progress:
...
async function main() {
  ...
  if (!command) {
    error('Usage: canopy-tools <command> [args] [--raw] [--cwd <path>]\nCommands: state, resolve-model, find-phase, commit, verify-summary, verify, frontmatter, template, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section, init');
  }
